function solution(boardArr, moveArr) {
  let answer = 0;

  let stack = [];

  for (const row of moveArr) {
    for (let column = 0; column < boardArr.length; column++) {
      let doll = boardArr[column][row - 1];
      if (doll !== 0) {
        if (stack[stack.length - 1] !== doll) {
          stack.push(doll);
          boardArr[column][row - 1] = 0;
        } else {
          stack.pop();
          answer += 2;
        }
 ...
 * 집어 올린 인 형은 바구니에 쌓이게 되는 데, 이때 바구니의 가장 아래 칸부터 인형이 순서대로 쌓이게 됩니다.
